...
const EditProfile = () => {
  ...
  const editProfileHandler = async () => {
    ...
    try {
      setLoading(true);
      const res = await axios.put(`/api/user/editprofile/profile`, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
        withCredentials: true,
  ...
  return (
    <div className="px-4 sm:px-6 md:px-8 py-6">
      ...
      <section className="w-full max-w-4xl mx-auto bg-zinc-900/60 border border-b-fuchsia-900 p-4 sm:p-6 rounded-xl shadow-xl">
        {/* Profile Row */}
        <div className="flex flex-col sm:flex-row justify-between items-center gap-6 mb-8 px-2 py-4 shadow-lg border border-white/10 rounded-lg">
        ...
        <div className="flex flex-col gap-4 text-white">
          {[
            ...
            { id: "username", label: "Username", value: input.username },
            ...
            <div className="flex flex-col gap-2" key={field.id}>
              ...
              <Textarea
                ...
                name={field.id}
                value={field.value}
                onChange={(e) => setInput({ ...input, [field.id]: e.target.value })}
                className="bg-zinc-800 border border-white/10 text-white font-light focus-visible:ring-transparent"
              />
            </div>
          ))}
          ...
          <div className="flex flex-col gap-2">
            ...
            <Select value={input.gender} onValueChange={selectChangeHandler}>
              ...
              <SelectContent>
                <SelectGroup className="bg-fuchsia-300 text-black">
                  <SelectItem value="male">Male</SelectItem>
                  <SelectItem value="female">Female</SelectItem>
                  <SelectItem value="other">Other</SelectItem>
                </SelectGroup>
        ...
        <div className="flex justify-center mt-8">
          {loading ? (
            <button
              disabled
              className="flex items-center px-6 py-2 bg-gradient-to-r from-green-600 to-teal-600 text-white font-semibold rounded-xl shadow-lg"
            >
              <Loader2 className="mr-2 h-4 w-4 animate-spin" />
              Please Wait...
            </button>
            ...
            <button
              onClick={editProfileHandler}
              className="px-6 py-2 bg-gradient-to-r from-green-600 to-teal-600 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition"
            >
              Submit
            </button>
          )}
